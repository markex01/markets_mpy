...
def merge_duplicate_columns(df):
    # Dictionary to collect columns by name
    col_groups = {}
    
    for col in df.columns:
        col_groups.setdefault(col, []).append(col)
    
    new_df = pd.DataFrame()
    
    for col, group in col_groups.items():
        if len(group) == 1:
            # No duplicates → keep original column
            new_df[col] = df[col]
        else:
            # Duplicate columns → sum them
            summed = df[group].fillna(0).sum(axis=1)
            new_df[col] = summed
