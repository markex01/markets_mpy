...
class InflationAdjuster:
    ...
    def __init__(
        ...
        if date_filter is None:
            # If no date_filter provided, we will use the latest value available in the DataFrame
            df = self.inflation_df.copy()
            latest_date = pd.to_datetime(df['Date'], format="%Y-%b").max().strftime("%Y-%b")
            self.date_filter = latest_date
        else:
    ...
    def transform_to_real_prices_yearly(
        ...
        Raises:
            ValueError: If no inflation data is found for the given country,
                date filter, and inflation type.
            KeyError: If a required annual rate is missing from the inflation map
                for any year encountered in ``df``.
        """
        df = df.copy()
        df['currency_year'] = df['currency'].str.extract(r'(\d{4})').astype(int)
    ...
    def get_conversion_factor(
        ...
        """Return the multiplication factor to convert a value from from_year to to_year prices.

        Args:
            from_year: Source year (the year the value is currently expressed in).
            to_year: Target year (the year to convert the value into).
            country: Country for inflation data (uses instance default if None).
            date_filter: Date filter for inflation data (uses instance default if None).

        Returns:
            Conversion factor such that ``value_in_to_year = value * factor``.

        Raises:
            ValueError: If no inflation data is found for the given country and date filter.
            KeyError: If a required annual rate is missing from the inflation map.
        """
        inflation_map = self._get_inflation_map(country, date_filter)
        return self.adjust_yearly(
    ...
    def create_conversion_table_all_countries(
        ...
        Args:
            ...
            export_path: If provided, path to save the resulting DataFrame as an Excel file
        ...
        # Set 'metric' as index
        result_df = result_df.set_index(['release', 'country', 'metric_type'])

        # Divide 'metric' column into 'country', 'release', and 'metric_type'
        result_df = result_df.reset_index()
        
        # Sort year columns
        year_cols = [col for col in result_df.columns if isinstance(col, int)]
        result_df = result_df[['release', 'country', 'metric_type'] + sorted(year_cols)]

        # Export to Excel if path provided
        if export_path:
            full_path = os.path.abspath(export_path)
            os.makedirs(os.path.dirname(full_path), exist_ok=True)
            result_df.to_excel(full_path)
